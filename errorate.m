...
fs = 2000;                % Sample rate (Hz)                                   %%采样率
pathDelays = [0 0.0001];  % Path delays (s)                                    %%信道延迟
pathPower = [0 -6];       % Path power (dB)                                    %%路径功率
fD = 5;                   % Maximum Doppler shift (Hz)                         %%最大
rchan = comm.RayleighChannel('SampleRate',fs, ...                              %%瑞利信道
